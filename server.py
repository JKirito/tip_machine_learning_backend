...
feature_names = ['Rainfall Amount', 'River Water Level', 'Soil Moisture Content', 'Temperature','Relative Humidity', 'Wind Speed', 'Topography','Urbanization Rate','Drainage System Capacity','Previous Flood History']
...
def shape_input_data(request_data):
    input_data = [
        ...
        request_data['soilMoistureContent'],
        request_data['temperature'],
        request_data['relativeHumidity'],
        request_data['windSpeed'],
        request_data['topography'],
        request_data['urbanizationRate'],
        request_data['drainageSystemCapacity'],
        request_data['previousFloodHistory'] 
    ]
    # print(input_data)
    return [input_data]
